...
# !pip -q install yfinance beautifulsoup4 lxml pandas numpy requests
...
SESSION.headers.update({"User-Agent": "Mozilla/5.0 (Colab; +https://colab.research.google.com)"})
TIMEOUT = 30
...
def safe_run(title: str, fn, default=None):
    """
    Runs fn() safely. If error -> prints stack trace and returns default.
    """
    log(f"\n=== {title} ===")
    try:
        out = fn()
...
def _safe_get(url: str, params: Optional[Dict[str, Any]] = None, expect_json: bool = True):
    r = SESSION.get(url, params=params, timeout=TIMEOUT)
    r.raise_for_status()
    return r.json() if expect_json else r.text


def _iss_to_df(payload: Dict[str, Any], block_name: str) -> pd.DataFrame:
...
def show_df(df: pd.DataFrame, name: str, n: int = 10) -> None:
    if df is None:
        log(f"[{name}] -> None")
        return
...
def moex_iss_dividends(secid: str) -> pd.DataFrame:
    url = f"https://iss.moex.com/iss/statistics/engines/stock/markets/shares/securities/{secid}/dividends.json"
...
def moex_iss_splits(secid: str) -> pd.DataFrame:
    url = f"https://iss.moex.com/iss/statistics/engines/stock/markets/shares/securities/{secid}/splits.json"
...
# -----------------------------
def yahoo_fundamentals(yahoo_ticker: str) -> dict:
...
# -----------------------------
def alphavantage_overview(symbol: str, api_key: str) -> dict:
...
# -----------------------------
def fmp_profile(symbol: str, api_key: str, limit: int = 40) -> pd.DataFrame:
    url = f"https://financialmodelingprep.com/api/v3/profile/{symbol}"
    j = _safe_get(url, params={"apikey": api_key}, expect_json=True)
    return pd.DataFrame(j).head(limit)
...
# -----------------------------
def eodhd_fundamentals(symbol_with_exchange: str, api_key: str) -> dict:
...
safe_run(
    ...
    lambda: {"profile": fmp_profile("AAPL", fmp_key).head(3), "ratios": fmp_ratios("AAPL", fmp_key, limit=5).head(3)}
    if fmp_key
