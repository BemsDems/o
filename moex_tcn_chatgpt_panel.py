"""Colab entrypoint for the MOEX TCN *panel* baseline (multi-ticker).

Usage (Colab):
  %run '/content/o/moex_tcn_chatgpt_panel.py'

Diff vs moex_tcn_chatgpt.py:
- Loads a panel dataset for multiple tickers (CFG["TICKERS"]).
- Builds windows strictly inside each ticker (no cross-ticker leakage).
- Thresholding/backtest is done per ticker, then aggregated.

Implementation lives in src/*; this file also handles packaging artifacts.
"""
...
from src.training.dataset_panel import prepare_dataset_once_panel
    ...
    with open(log_path, "w", encoding="utf-8") as log_f:
        ...
        try:
            with contextlib.redirect_stdout(tee_out), contextlib.redirect_stderr(tee_err):
                ...
                # Force-reload train module to ensure run_once signature is fresh.
                import importlib
                import src.training.train_panel as _train

                _train = importlib.reload(_train)
                run_once_panel = _train.run_once_panel

                results = []
                for sd in CFG.get("RUN_SEEDS", [42]):
                    res = run_once_panel(int(sd), prepared, run_dir=run_dir)
                    results.append(res)
                    print(
                        f"Seed {sd}: "
