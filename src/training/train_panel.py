...
def run_once_panel(
    ...
    """Run one full train/eval cycle for a given seed on a multi-ticker panel dataset."""
    ...
    dw = prepared["dw"]
    group_w = prepared["group_w"]
    test_mask = prepared["test_mask"]
    val_mask = prepared["val_mask"]
    future_ret_w = prepared["future_ret_w"]
    px_map = prepared["px_map"]

    ckpt_path = f"tcn_panel_best_seed_{int(run_seed)}.weights.h5"

    model = build_tcn_model(int(CFG["WINDOW"]), int(X_train.shape[2]), float(CFG["LR"]))
    ...
    bt_f1 = {
        "alpha_mean": float(bt_f1_tbl["alpha"].mean()) if not bt_f1_tbl.empty else 0.0,
        "n_trades_total": int(bt_f1_tbl["n_trades"].sum()) if not bt_f1_tbl.empty else 0,
    }
    bt_pnl = {
        "alpha_mean": float(bt_pnl_tbl["alpha"].mean()) if not bt_pnl_tbl.empty else 0.0,
        "n_trades_total": int(bt_pnl_tbl["n_trades"].sum()) if not bt_pnl_tbl.empty else 0,
    }

    print("\n" + "=" * 70)
    print("=== COMPACT DASHBOARD (PANEL) ===")
    print("=" * 70)
    print(
    ...
    print("\nBACKTEST TEST (panel, mean alpha across tickers):")
    print(f"thr_f1={thr_f1:.2f} | alpha_mean={bt_f1['alpha_mean']:+.2%} trades_total={bt_f1['n_trades_total']}")
    print(f"thr_pnl={thr_pnl:.2f} | alpha_mean={bt_pnl['alpha_mean']:+.2%} trades_total={bt_pnl['n_trades_total']}")
